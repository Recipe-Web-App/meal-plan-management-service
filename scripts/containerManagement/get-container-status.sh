...
if kubectl get ingress meal-plan-management-ingress -n "$NAMESPACE" >/dev/null 2>&1; then
    kubectl get ingress meal-plan-management-ingress -n "$NAMESPACE"

    # Check if ingress has IP assigned
    INGRESS_IP=$(kubectl get ingress meal-plan-management-ingress -n "$NAMESPACE" -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null || echo "")
