...
CONFIG_DIR="k8s"
SECRET_NAME="meal-plan-management-secrets" # pragma: allowlist secret
...
print_separator "="
echo -e "${CYAN}📦 Deploying Meal Plan Management Service container...${NC}"
print_separator "-"

kubectl apply -f "${CONFIG_DIR}/deployment.yaml"

print_separator "="
echo -e "${CYAN}🌐 Exposing Meal Plan Management Service via ClusterIP Service...${NC}"
print_separator "-"
...
print_separator "="
echo -e "${CYAN}📊 Applying Horizontal Pod Autoscaler...${NC}"
print_separator "-"
...
kubectl wait --namespace="$NAMESPACE" \
  ...
  --selector=app=meal-plan-management-service \
  --timeout=90s

print_separator "-"
print_status "ok" "Meal Plan Management Service is up and running in namespace '$NAMESPACE'."

print_separator "="
echo -e "${CYAN}🔗 Setting up /etc/hosts for meal-plan-management.local...${NC}"
print_separator "-"

MINIKUBE_IP=$(minikube ip)
if grep -q "meal-plan-management.local" /etc/hosts; then
  echo -e "${YELLOW}🔄 Updating /etc/hosts for meal-plan-management.local...${NC}"
  sed -i "/meal-plan-management.local/d" /etc/hosts
else
  echo -e "${YELLOW}➕ Adding meal-plan-management.local to /etc/hosts...${NC}"
fi
echo "$MINIKUBE_IP meal-plan-management.local" | tee -a /etc/hosts
...
echo "  Gateway Hosts: meal-plan-management.local, sous-chef-proxy.local"
