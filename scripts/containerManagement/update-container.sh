...
echo "🔄 Updating Meal Plan Management Service..."
print_separator "="
...
print_separator
echo -e "${CYAN}🟢 Rebuilding Node.js Docker image: ${FULL_IMAGE_NAME}${NC}"
...
kubectl rollout status deployment/meal-plan-management-service -n "$NAMESPACE" --timeout=120s
...
print_separator "="
print_status "ok" "Meal Plan Management Service updated successfully!"
echo -e "${CYAN}🌍 Access at: http://meal-plan-management.local/api/v1/meal-plan-management/health${NC}"
